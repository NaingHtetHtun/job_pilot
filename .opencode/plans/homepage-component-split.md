# Plan: Refactor app/page.tsx — Extract Inline Components

## Goal

Move all inline component definitions from `app/page.tsx` into separate files under `components/layout/` and `components/homepage/`, then rewrite `app/page.tsx` to only import and compose. Update agent context files to enforce this pattern going forward.
...
```
mkdir -p components/layout components/homepage
```

(Already done — directories exist.)

---

## Step 2 — Create 9 Component Files
...
Replace all 349 lines with pure composition:
...
## Step 4 — Update `context/code-standards.md`

Add a new rule under the "Component Structure" section (after line 97):
...
- Data arrays and mock data live in the component file where they're used, not in page files
...
## Step 5 — Update `context/ui-registry.md`

The existing Landing* entries (Navbar, Hero, Split Feature Panel, Testimonial, CTA Banner, Footer) reference files that don't exist yet. Update the "Last updated" dates to today (2026-06-22) and verify the class tables match the extracted code. Replace the `Split Feature Panel` entry with separate entries for `HowItWorks` and `Features` since they're now separate files.

Update the `Landing Navbar` entry to reflect the actual inline code values — `max-w-[1080px]` instead of `max-w-[1440px]`, `rounded-md` on CTA, etc.
...
Both should pass clean. The only differences from the original code are:
1. Component definitions live in separate files
2. `app/page.tsx` is reduced from 349 lines to ~25 lines
3. `<SuccessStory>` uses `&ldquo;` / `&rdquo;` instead of `&ldquo;` / `&rdquo;` — visually identical
4. `HowItWorks` replaces `FeatureSection`, `Features` replaces `ConfidenceSection`, `CTASection` replaces `BottomCta`
