...
- [x] 01 Homepage
- [x] 02 Auth (rebuilt — was missing on disk despite being checked; now fully implemented)
...
- AGENTS.md updated: replaced MCP-based InsForge docs section (fetch-docs, fetch-sdk-docs, etc.) with skill-based instructions — load the `insforge` skill for SDK docs, `insforge-cli` for CLI operations.
- Auth implemented with `@insforge/sdk/ssr` SSR helpers: `createBrowserClient()`, `createServerClient({ cookies })`, `createAuthActions()` for OAuth initiation/exchange, `createRefreshAuthRouter()` for refresh, `updateSession()` from `@insforge/sdk/ssr/middleware` in `proxy.ts`.
- Architecture docs and library-docs fixed: `@insforge/ssr` → `@insforge/sdk/ssr`; `createServerClient(url, key, { cookies })` → `createServerClient({ cookies })` with implicit env-var reading.
...
- Feature 04 verified end-to-end: both migrations applied (`initial-schema` + `fix-auth-trigger-raw-user-meta-data`), 4 tables + 12 RLS policies confirmed via CLI. `resumes` storage bucket exists. TypeScript types match DB schema.
...
- 2026-06-22: Tailwind v4 global styling setup completed. `app/globals.css` now defines all JobPilot semantic tokens with `@theme`, adds generated `shadow-card` and landing shadow tokens, applies base page styles, and restores shared landing helpers (`landing-panel`, `landing-grid`, `landing-hero-glow`, `landing-divider`, `landing-button-primary`, `landing-button-secondary`). `app/layout.tsx` now uses `Inter` from `next/font/google` with `variable: "--font-sans"` so the global font token resolves correctly.
...
- Feature 04: four tables (`profiles`, `agent_runs`, `jobs`, `agent_logs`) created via InsForge CLI migrations with full RLS (12 table policies + 4 storage object policies). Two Postgres triggers on `profiles`: `on_profile_updated` auto-sets `updated_at`; `on_auth_user_created` on `auth.users` auto-inserts a minimal profile row on signup. `resumes` private storage bucket created with path-scoped RLS on `storage.objects`. `types/index.ts` created with `Profile`, `WorkExperience`, `Education`, `AgentRun`, `Job`, `AgentLog` interfaces. Fix-auth-trigger migration applied to use `new.profile ->> 'name'` instead of `new.raw_user_meta_data ->> 'full_name'`. All verified via CLI `db query`.
